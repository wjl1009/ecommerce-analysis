...
当面试官问"你做过什么数据分析项目"时，用这个结构回答：

1. **业务背景**："这是一个巴西电商平台的数据，我分析了 10 万笔订单"
2. **分析框架**："我做了 5 个模块的分析——用户分群、漏斗、留存、品类、A/B测试"
3. **关键发现**："高价值用户只占 x% 但贡献了 x% 的收入；最大转化损失在 x 环节；第 1 个月是留存黄金窗口"
4. **工具方法**："SQL 做数据提取和分群，Python 做统计建模和可视化"
5. **业务建议**："我建议对流失风险用户推送优惠券，对高价值用户建立 VIP 体系"
